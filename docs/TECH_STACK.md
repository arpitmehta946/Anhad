...
- **Anti-cheat rate limiting:** `INCR ratelimit:taps:<user_id>` with a 60-second TTL; sessions exceeding ~200 taps/minute or showing unnaturally uniform tap timing get flagged. This cross-batch window only applies to batches recorded within the last few minutes ("live" chanting) — a delayed/backlog flush (client was offline, auth had lapsed, anything that holds up a sync) already proved its honest pace via the per-batch check using its own recorded timestamps, and would otherwise get bulk-rejected purely for arriving in one burst on reconnect.
